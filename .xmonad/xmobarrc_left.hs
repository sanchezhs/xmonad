Config { font            = "xft:Nerd:pixelsize=14:antialias=true:hinting=true"
       ...
       , sepChar = "%"
       , alignSep = "}{"
       , template = " %UnsafeStdinReader% }{  <fc=#5e81ac><action=`alacritty -e htop`>%cpu%</action></fc> <fc=#8fbcbb> | </fc> <fc=#5e81ac><action=`alacritty -e htop`>%memory%</action></fc> <fc=#8fbcbb> | </fc> <fc=#5e81ac>%disku%</fc> <fc=#8fbcbb> | </fc> <fc=#5e81ac><action=`emacsclient -c -a 'emacs' --eval '(doom/window-maximize-buffer(dt/year-calendar))'`>%date%</action></fc>"
       }
